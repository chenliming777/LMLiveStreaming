...
NSString *const LMAudioComponentFailedToCreateNotification = @"LMAudioComponentFailedToCreateNotification";

@interface LMAudioCapture ()

@property (nonatomic, assign) AudioComponentInstance    componetInstance;
@property (nonatomic, assign) AudioComponent            component;
@property (nonatomic, strong) dispatch_queue_t       taskQueue;
@property (nonatomic, assign) BOOL isRunning;
@property (nonatomic, strong) LMAudioStreamingConfiguration *configuration;

@end

@implementation LMAudioCapture

#pragma mark -- LiftCycle
- (instancetype)initWithAudioConfiguration:(LMAudioStreamingConfiguration *)configuration{
    if(self = [super init]){
...
- (void)handleAudioComponentCreationFailure {
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:LMAudioComponentFailedToCreateNotification object:nil];
    });
}
...
static OSStatus handleInputBuffer(void *inRefCon,
    ...
    @autoreleasepool {
        LMAudioCapture *source = (__bridge LMAudioCapture *)inRefCon;
        if(!source) return -1;
