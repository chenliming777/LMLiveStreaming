...
@interface LMHardwareAudioEncoder (){
    AudioConverterRef m_converter;
    char *aacBuf;
}
@property (nonatomic, strong) LMAudioStreamingConfiguration *configuration;
@property (nonatomic, weak) id<LMAudioEncodeDelegate> aacDeleage;

@end

@implementation LMHardwareAudioEncoder

- (instancetype)initWithAudioStreamConfiguration:(LMAudioStreamingConfiguration *)configuration{
    if(self = [super init]){
...
- (void)setDelegate:(id<LMAudioEncodeDelegate>)delegate{
...
- (void)encodeAudioData:(AudioBufferList)inBufferList timeStamp:(uint64_t)timeStamp{
    ...
    LMAudioFrame *audioFrame = [LMAudioFrame new];
